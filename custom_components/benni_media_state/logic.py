...
# --------------------------------------------------------------------------- #
# Anstiegs-Latch (FLEET-Workaround, HA-frei + testbar)
# --------------------------------------------------------------------------- #
@dataclass(frozen=True)
class LatchResult:
    """Ergebnis eines Anstiegs-Latch-Schritts.

    active:     effektiver Signal-Wert (nach Latch).
    latched:    neuer persistenter Latch-Zustand (in den nächsten Schritt geben).
    rise_since: Zeitpunkt der ersten True-Beobachtung des aktuellen Pulses
                (None, sobald raw=False) — in den nächsten Schritt geben.
    recheck_in: Sekunden bis zur fälligen Re-Evaluation, oder None wenn keine
                nötig ist (Aufrufer plant den Timer; das Roh-Signal liefert
                während eines anhaltenden Pulses kein neues Event).
    """

    active: bool
    latched: bool
    rise_since: Optional["datetime"]
    recheck_in: Optional[float]


def latch_rising_edge(
    raw: bool,
    *,
    latched: bool,
    rise_since: Optional["datetime"],
    ...
    """Reine Anstiegs-Flanken-Verzögerung.

    `raw` muss `latch_seconds` DURCHGEHEND True sein, bevor `active=True` wird.
    Fällt `raw` vorher auf False, beginnt das Fenster beim nächsten True neu
    (kurzer Standby-Puls wird verschluckt). Die fallende Flanke folgt `raw`
    sofort (kein Halte-Effekt — das macht core_devices' sticky_hold bereits).
    """
    if not raw:
        return LatchResult(active=False, latched=False, rise_since=None, recheck_in=None)
    if latched:
        return LatchResult(active=True, latched=True, rise_since=rise_since, recheck_in=None)
    if latch_seconds <= 0:
        return LatchResult(active=True, latched=True, rise_since=rise_since, recheck_in=None)
    start = rise_since or now
    elapsed = (now - start).total_seconds()
    if elapsed >= latch_seconds:
        return LatchResult(active=True, latched=True, rise_since=start, recheck_in=None)
    return LatchResult(
        active=False, latched=False, rise_since=start, recheck_in=latch_seconds - elapsed
    )
